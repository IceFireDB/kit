...
func (c *Client) CreateInOrder(path string, data []byte) (string, error) {
	...
	path = filepath.Join(path, "prefix_")
	log.Debugf("zkclient create node %s", path)
	err := c.shell(func(conn *zk.Conn) error {
		_, err := c.create(conn, path, data, zk.FlagSequence)
		return err
	})
	if err != nil {
		log.Debugf("zkclient create node %s failed: %s", path, err)
...
func (c *Client) DeleteRecursive(path string, version int) error {
	// version: -1 delete any version of the node at path - only applies to the top node
	err := c.shell(func(conn *zk.Conn) error {
		err := conn.Delete(path, int32(version))
		if err != nil && errors.NotEqual(err, zk.ErrNoNode) {
			return errors.Trace(err)
		}
		return nil
	...
	if errors.NotEqual(err, zk.ErrNotEmpty) {
		return err
	}
	...
	err = c.shell(func(conn *zk.Conn) error {
		_, err = conn.SetACL(path, zk.WorldACL(zk.PermAdmin|zk.PermDelete|zk.PermRead), int32(version))
		return nil
	})
	if err != nil {
		return err
	}
	children, err := c.List(path, true)
	if err != nil {
		return err
	}
	for _, child := range children {
		err := c.DeleteRecursive(child, version)
		if err != nil && errors.NotEqual(err, zk.ErrNoNode) {
	...
	if err != nil && errors.NotEqual(err, zk.ErrNotEmpty) {
		err = fmt.Errorf("zkutil: nodes getting recreated underneath delete (app race condition): %v", path)
	}
	return err
}
...
func (c *Client) Read(path string, must bool) ([]byte, error) {
	...
	err := c.shell(func(conn *zk.Conn) error {
		...
		if err != nil {
			...
			return errors.Errorf("read node %s failed: %w", path, err)
		}
		data = b
		return nil
	})
...
func (c *Client) WatchInOrder(path string) (<-chan client.Event, []string, error) {
	...
	err := c.shell(func(conn *zk.Conn) error {
		...
		signal = make(chan client.Event, 1)
		go func() {
			defer close(signal)
			e := <-w
			ev := client.Event{
